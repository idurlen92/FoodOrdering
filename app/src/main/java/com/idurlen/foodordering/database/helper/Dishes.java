...
public class Dishes extends HelperMethods{
	...
	public static final String COL_DISH_TYPE_ID = "dish_type_id";
	...
	public static String getCreateTableStatement(){
		return "CREATE TABLE " + TABLE_NAME + "(" +
				...
				COL_DISH_TYPE_ID + " INTEGER REFERENCES " + DishTypes.TABLE_NAME +
	...
	public static void prepareTestData(SQLiteDatabase db){
		...
		for(int i = 1; i < 41; i++) {
			for(int j = 1; j < astrTitles.length; j++) {
				...
				values.put(COL_DISH_TYPE_ID, (j % 8) + 1);
	...
	public static List<Dish> getDishesOfRestaurant(SQLiteDatabase db, int restaurantId){
		final String strStatement = "SELECT * FROM " + TABLE_NAME +
				...
				" ORDER BY " + COL_DISH_TYPE_ID + ", " + COL_TITLE;
	...
	public static void insertDishes(SQLiteDatabase db, List<Object> lDishes){
		ContentValues values = new ContentValues();

		for(Object obj : lDishes){
			Dish dish = (Dish) obj;

			values.put(COL_ID, dish.getId());
			values.put(COL_DISH_TYPE_ID, dish.getDishTypeId());
			values.put(COL_RESTAURANT_ID, dish.getRestaurantId());
			values.put(COL_TITLE, dish.getTitle());
			values.put(COL_PRICE, dish.getPrice());
			values.put(COL_DESCRIPTION, dish.getDescription());
